...
`;
  ...
  return {
    ...
    datasetsPreview: {
      ...
      ADCM: (clientRealData.ADCM || []).slice(0, 10),
      ADMH: (clientRealData.ADMH || []).slice(0, 10),
      ADTTE: (clientRealData.ADTTE || []).slice(0, 10),
      ADEFF: (clientRealData.ADEFF || []).slice(0, 10),
      DM: dm.slice(0, 10),
      VS: vs.slice(0, 10),
      LB: lb.slice(0, 10),
      AE: ae.slice(0, 10),
      EX: ex.slice(0, 10),
      CM: (clientRealData.CM || []).slice(0, 10),
      MH: (clientRealData.MH || []).slice(0, 10),
      DS: (clientRealData.DS || []).slice(0, 10),
      EG: (clientRealData.EG || []).slice(0, 10),
      QS: (clientRealData.QS || []).slice(0, 10),
      SV: (clientRealData.SV || []).slice(0, 10),
      TS: (clientRealData.TS || []).slice(0, 10)
    }
  };
}
...
// ============================================================================
// CLINICALOPS v8.0: COMMAND BAR, DOMAIN VERIFICATION & ISSUE MANAGEMENT
// ============================================================================

window.currentIssueTabFilter = window.currentIssueTabFilter || 'ALL';

function switchIssueTab(filterMode, btnEl) {
  window.currentIssueTabFilter = filterMode;
  document.querySelectorAll('.issue-tab-btn').forEach(b => b.classList.remove('active'));
  if (btnEl) btnEl.classList.add('active');
  else {
    const defaultBtn = document.getElementById('tab-issues-' + String(filterMode).toLowerCase().replace('_', ''));
    if (defaultBtn) defaultBtn.classList.add('active');
  }
  renderIssueManagementSystem();
}

function renderIssueManagementSystem(targetDomain) {
  const container = document.getElementById('qc-findings-container');
  const fixedSection = document.getElementById('fixed-issues-section');
  const reviewSection = document.getElementById('review-required-section');
  if (!container) return;

  // Gather issues across active domain or study
  let allFindings = [];
  const activeDom = targetDomain || window.currentViewingDataset;
  
  if (activeDom && window.clientAuditLogs && window.clientAuditLogs[activeDom] && window.clientAuditLogs[activeDom].length > 0) {
    allFindings = window.clientAuditLogs[activeDom].map(f => ({ ...f, domain: f.domain || activeDom }));
  } else if (window.clientAuditLogs) {
    Object.keys(window.clientAuditLogs).forEach(dom => {
      const logs = window.clientAuditLogs[dom] || [];
      logs.forEach(f => allFindings.push({ ...f, domain: f.domain || dom }));
    });
  }

  // Include cross-domain study issues if present
  if (window.studyCrossDomainIssues && Array.isArray(window.studyCrossDomainIssues)) {
    allFindings = allFindings.concat(window.studyCrossDomainIssues);
  }

  // Calculate counts for the 6 tabs
  const countAll = allFindings.length;
  const countOpen = allFindings.filter(f => f.status === 'OPEN' || (!f.status && f.severity !== 'PASS' && f.severity !== 'INFO')).length;
  const countFixed = allFindings.filter(f => f.status === 'FIXED').length;
  const countWarnings = allFindings.filter(f => f.severity === 'WARNING' || f.status === 'WARNING').length;
  const countReview = allFindings.filter(f => f.status === 'REVIEW_REQUIRED' || f.severity === 'REVIEW_REQUIRED').length;
  const countUnresolved = allFindings.filter(f => f.status === 'UNRESOLVED' || (f.status === 'OPEN' && !f.newVal)).length;

  // Update count pills
  const pillAll = document.getElementById('issue-count-all');
  if (pillAll) pillAll.textContent = countAll;
  const pillOpen = document.getElementById('issue-count-open');
  if (pillOpen) pillOpen.textContent = countOpen;
  const pillFixed = document.getElementById('issue-count-fixed');
  if (pillFixed) pillFixed.textContent = countFixed;
  const pillWarnings = document.getElementById('issue-count-warnings');
  if (pillWarnings) pillWarnings.textContent = countWarnings;
  const pillReview = document.getElementById('issue-count-review');
  if (pillReview) pillReview.textContent = countReview;
  const pillUnresolved = document.getElementById('issue-count-unresolved');
  if (pillUnresolved) pillUnresolved.textContent = countUnresolved;

  const currentFilter = window.currentIssueTabFilter || 'ALL';

  // Toggle dedicated sections
  if (currentFilter === 'FIXED') {
    if (reviewSection) reviewSection.style.display = 'none';
    if (fixedSection) {
      fixedSection.style.display = 'block';
      const fixedItems = allFindings.filter(f => f.status === 'FIXED');
      if (fixedItems.length === 0) {
        fixedSection.innerHTML = '<div style="padding:16px; background:rgba(255,255,255,0.03); border:1px dashed rgba(255,255,255,0.1); border-radius:6px; color:var(--text-muted); text-align:center; font-size:12px;">No deterministic fixes have been applied yet. Run validation or execute automated repair.</div>';
      } else {
        fixedSection.innerHTML = `
          <div style="background:rgba(34, 197, 94, 0.08); border:1px solid rgba(34, 197, 94, 0.3); border-radius:6px; padding:12px 16px; margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <strong style="color:#4ade80; font-size:13px;">✨ Deterministic &amp; Controlled Terminology Fixes Applied (${fixedItems.length})</strong>
              <span style="font-size:11px; color:var(--text-muted);">Source data preserved 100% unaltered (21 CFR Part 11 Audit Trail)</span>
            </div>
          </div>
          <div class="fixed-issues-table-wrap">
            <table class="fixed-issues-table">
              <thead>
                <tr>
                  <th>Domain</th>
                  <th>Row</th>
              ...
              <tbody>
                ${fixedItems.map(item => `
                  <tr>
                    <td><span class="badge-domain">${escapeHtml(item.domain || 'DM')}</span></td>
                    <td>Row ${item.row || 1}</td>
                    <td><strong>${escapeHtml(item.usubjid || item.subject || '—')}</strong></td>
                    <td><span class="badge-variable">${escapeHtml(item.variable || item.column || '—')}</span></td>
                    <td class="diff-val-old">${escapeHtml(String(item.oldVal === '' ? '(blank)' : (item.oldVal !== undefined ? item.oldVal : '(blank)')))}</td>
                    <td class="diff-val-new">${escapeHtml(String(item.newVal || '—'))}</td>
                    <td><code style="font-size:11px; color:#38bdf8;">${escapeHtml(item.rule || item.errorType || 'CDISC')}</code></td>
                    <td style="font-size:11px; color:#cbd5e1;">${escapeHtml(item.justification || item.error || 'Deterministic derivation')}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
  ...
  if (currentFilter === 'REVIEW_REQUIRED') {
    if (fixedSection) fixedSection.style.display = 'none';
    if (reviewSection) {
      reviewSection.style.display = 'block';
      const reviewItems = allFindings.filter(f => f.status === 'REVIEW_REQUIRED' || f.severity === 'REVIEW_REQUIRED');
      if (reviewItems.length === 0) {
        reviewSection.innerHTML = '<div style="padding:16px; background:rgba(255,255,255,0.03); border:1px dashed rgba(255,255,255,0.1); border-radius:6px; color:var(--text-muted); text-align:center; font-size:12px;">🎉 Zero items currently require human clinical review. All records conform to deterministic rules.</div>';
      } else {
        reviewSection.innerHTML = `
          <div style="background:rgba(245, 158, 11, 0.1); border:1px solid rgba(245, 158, 11, 0.35); border-radius:6px; padding:12px 16px; margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <strong style="color:#fbbf24; font-size:13px;">🔍 Clinical &amp; Medical Review Required (${reviewItems.length})</strong>
              <span style="font-size:11px; color:var(--text-muted);">Deterministic guessing strictly prohibited by GxP standards</span>
            </div>
            <div style="font-size:11.5px; color:#fef08a; margin-top:4px;">
              The following data points exhibit ambiguity (e.g. unmapped terms, conflicting dates, unverified MedDRA codings). A clinical data manager or medical monitor must review these entries.
            </div>
          </div>
          ${reviewItems.map(item => `
            <div class="review-reasoning-card">
              <div class="review-reasoning-header">
                <span>⚠️ [${escapeHtml(item.domain || 'STUDY')}] ${escapeHtml(item.variable || item.errorType || 'ISSUE')} (Row ${item.row || '—'})</span>
                <span style="font-size:10.5px; background:rgba(245, 158, 11, 0.2); padding:2px 8px; border-radius:4px;">${escapeHtml(item.usubjid || item.subject || 'Subject')}</span>
              </div>
              <div class="review-reasoning-text">
                <strong>Finding:</strong> ${escapeHtml(item.error || 'Ambiguous clinical value')}<br>
                <strong>Current Value:</strong> <code>${escapeHtml(String(item.oldVal || '(blank)'))}</code><br>
                <strong>Reasoning:</strong> ${escapeHtml(item.justification || 'Value cannot be deterministically inferred without medical records or protocol amendment.')}
              </div>
            </div>
          `).join('')}
        `;
  ...
  // For ALL, OPEN, WARNINGS, UNRESOLVED:
  if (fixedSection) fixedSection.style.display = 'none';
  if (reviewSection) reviewSection.style.display = 'none';

  let filtered = allFindings;
  if (currentFilter === 'OPEN') {
    filtered = allFindings.filter(f => f.status === 'OPEN' || (!f.status && f.severity !== 'PASS' && f.severity !== 'INFO'));
  } else if (currentFilter === 'WARNINGS') {
    filtered = allFindings.filter(f => f.severity === 'WARNING' || f.status === 'WARNING');
  } else if (currentFilter === 'UNRESOLVED') {
    filtered = allFindings.filter(f => f.status === 'UNRESOLVED' || (f.status === 'OPEN' && !f.newVal));
  }

  if (filtered.length === 0) {
    container.innerHTML = `<div style="padding:24px; text-align:center; color:var(--text-muted); font-size:12.5px;">✅ No issues found in current filter category (${currentFilter}).</div>`;
    return;
  }

  container.innerHTML = filtered.map(f => {
    const statusClass = f.status === 'FIXED' ? 'status-fixed' :
                        f.status === 'REVIEW_REQUIRED' ? 'status-review' :
                        f.severity === 'WARNING' ? 'status-warning' :
                        f.status === 'UNRESOLVED' ? 'status-unresolved' : 'status-open';

    const statusBadgeClass = f.status === 'FIXED' ? 'status-badge-fixed' :
                             f.status === 'REVIEW_REQUIRED' ? 'status-badge-review' :
                             f.severity === 'WARNING' ? 'status-badge-warning' :
                             f.status === 'UNRESOLVED' ? 'status-badge-unresolved' : 'status-badge-open';

    const hasDiff = f.newVal && f.oldVal !== f.newVal;

    return `
      <div class="issue-card-v8 ${statusClass}">
        <div class="issue-card-header">
          <div class="issue-locator-row">
            <span class="badge-domain">${escapeHtml(f.domain || 'DM')}</span>
            <span class="badge-variable">${escapeHtml(f.variable || f.column || 'GENERAL')}</span>
            <span class="badge-usubjid">${escapeHtml(f.usubjid || f.subject || 'All Subjects')}</span>
            <span style="font-size:11px; color:var(--text-muted);">Row ${f.row || 1}</span>
          </div>
          <div style="display:flex; align-items:center; gap:6px;">
            <span class="issue-status-badge ${statusBadgeClass}">${escapeHtml(f.status || f.severity || 'ERROR')}</span>
          </div>
        </div>

        <div class="issue-card-desc">
          <strong>${escapeHtml(f.errorType || f.rule || 'RULE')}:</strong> ${escapeHtml(f.error || f.message || '')}
        </div>

        ${hasDiff ? `
          <div class="issue-card-diff">
            <span class="diff-val-old">${escapeHtml(String(f.oldVal === '' ? '(blank)' : f.oldVal))}</span>
            <span class="diff-arrow">➔</span>
            <span class="diff-val-new">${escapeHtml(String(f.newVal))}</span>
          </div>
        ` : ''}

        <div class="issue-card-meta-grid">
          <div class="issue-meta-item">
            <strong>Rule:</strong> ${escapeHtml(f.rule || 'CDISC SDTMIG/ADaMIG Standard')}
          </div>
          <div class="issue-meta-item">
            <strong>Category:</strong> ${escapeHtml(f.category || 'DATA_CONFORMANCE')}
          </div>
          <div class="issue-meta-item" style="grid-column: 1 / -1;">
            <strong>Justification:</strong> ${escapeHtml(f.justification || f.method || 'Standard CDISC rule evaluation.')}
          </div>
        </div>
      </div>
    `;
...
function executeClinicalCommand(cmdText) {
  const inputEl = document.getElementById('clinical-command-input');
  const text = (cmdText || (inputEl ? inputEl.value : '')).trim();
  if (!text) return;

  const parseFn = typeof parseClinicalCommand === 'function' 
    ? parseClinicalCommand 
    : (window.parseClinicalCommand || (window.ClinicalValidationOrchestrator && window.ClinicalValidationOrchestrator.parseClinicalCommand));

  const parsed = parseFn ? parseFn(text) : { intent: 'UNKNOWN', raw: text };
  ...
  if (intent === 'STUDY_MAP' || /study\s*map/i.test(text)) {
  ...
  } else if (intent === 'SHOW_OPEN_ERRORS') {
    switchTab('tab-qc');
    switchIssueTab('OPEN', document.getElementById('tab-issues-open'));
  } else if (intent === 'SHOW_FIXED_ISSUES') {
    switchTab('tab-qc');
    switchIssueTab('FIXED', document.getElementById('tab-issues-fixed'));
  } else if (intent === 'SHOW_WARNINGS') {
    switchTab('tab-qc');
    switchIssueTab('WARNINGS', document.getElementById('tab-issues-warnings'));
  } else if (intent === 'SHOW_REVIEW_REQUIRED') {
    switchTab('tab-qc');
    switchIssueTab('REVIEW_REQUIRED', document.getElementById('tab-issues-review'));
  } else if (intent === 'SHOW_ALL_ERRORS') {
    switchTab('tab-qc');
    switchIssueTab('ALL', document.getElementById('tab-issues-all'));
  } else if (intent === 'SHOW_CORRECTIONS') {
    if (typeof openCorrectionReviewPanel === 'function') openCorrectionReviewPanel();
  } else if (intent === 'GENERATE_CORRECTED_DATASET') {
    if (typeof generateCleanCorrectedDataset === 'function') generateCleanCorrectedDataset();
  } else if (intent === 'RUN_DOUBLE_PROGRAMMING') {
    switchTab('tab-qc');
    ...
    if (['DM','AE','ADAE','ADSL','LB','ADLB','VS','ADVS','EX','CM','DS','SV','MH','EG'].includes(upper)) {
      triggerDeepVerifyDomain(upper);
    } else {
      triggerDeepVerifyDomain();
    }
  }
}

function executeQuickCommand(cmdText) {
  const inputEl = document.getElementById('clinical-command-input');
  if (inputEl) inputEl.value = cmdText;
  executeClinicalCommand(cmdText);
}

function triggerDeepVerifyDomain(domain) {
  const targetDomain = domain || window.currentViewingDataset || 'DM';
  const rawRows = (window.clientSourceData && window.clientSourceData[targetDomain]) ||
                  (window.clientRealData && window.clientRealData[targetDomain]) ||
                  (window.clientRealData && window.clientRealData.CUSTOM);

  if (!rawRows || !Array.isArray(rawRows) || rawRows.length === 0) {
    if (typeof showSelfHealingNotification === 'function') {
      showSelfHealingNotification('No rows loaded for domain ' + targetDomain + '. Please upload or select a domain with data.', 'warning');
    }
    return;
  }

  const orch = typeof ClinicalValidationOrchestrator !== 'undefined' 
    ? ClinicalValidationOrchestrator 
    : (window.ClinicalValidationOrchestrator || null);

  let audit;
  if (orch && typeof orch.validateDataset === 'function') {
    audit = orch.validateDataset(targetDomain, rawRows);
  } else {
    audit = verifyAndRepairClinicalData(targetDomain, rawRows);
  }

  if (!window.clientAuditLogs) window.clientAuditLogs = {};
  window.clientAuditLogs[targetDomain] = audit.auditLog || [];
  if (!window.clientCorrectedData) window.clientCorrectedData = {};
  window.clientCorrectedData[targetDomain] = audit.repairedRows || audit.cleanRows || [];
  if (!window.clientColumnProfiles) window.clientColumnProfiles = {};
  window.clientColumnProfiles[targetDomain] = audit.columnProfiles || {};

  if (typeof renderDatasetTable === 'function') renderDatasetTable(targetDomain);
  renderIssueManagementSystem(targetDomain);

  if (typeof showSelfHealingNotification === 'function') {
    const fixedCount = (audit.auditLog || []).filter(a => a.status === 'FIXED').length;
    showSelfHealingNotification('[DEEP VERIFY ' + targetDomain + '] Audited ' + rawRows.length + ' rows (' + (audit.auditLog || []).length + ' issues found, ' + fixedCount + ' fixed deterministically).', 'success');
  }
}

function triggerVerifyCompleteStudy() {
  const datasetsMap = window.clientSourceData || window.clientRealData || {};
  const activeKeys = Object.keys(datasetsMap).filter(k => Array.isArray(datasetsMap[k]) && datasetsMap[k].length > 0);

  if (activeKeys.length === 0) {
    if (typeof showSelfHealingNotification === 'function') {
      showSelfHealingNotification('No clinical datasets loaded. Ingest data to run complete study verification.', 'warning');
    }
    return;
  }
  ...
  if (orch && typeof orch.validateStudy === 'function') {
    const studyRes = orch.validateStudy(datasetsMap);
    window.studyCrossDomainIssues = studyRes.crossDomainIssues || [];
    
    Object.keys(studyRes.datasetResults || {}).forEach(dom => {
      const dRes = studyRes.datasetResults[dom];
      if (!window.clientAuditLogs) window.clientAuditLogs = {};
      window.clientAuditLogs[dom] = dRes.auditLog || [];
      if (!window.clientCorrectedData) window.clientCorrectedData = {};
      window.clientCorrectedData[dom] = dRes.repairedRows || [];
      if (!window.clientColumnProfiles) window.clientColumnProfiles = {};
      window.clientColumnProfiles[dom] = dRes.columnProfiles || {};
    });
    ...
    if (typeof showSelfHealingNotification === 'function') {
      showSelfHealingNotification('[STUDY VERIFICATION] Audited ' + activeKeys.length + ' domains with ' + window.studyCrossDomainIssues.length + ' cross-domain findings.', 'success');
    }
  } else {
    activeKeys.forEach(dom => {
      const audit = verifyAndRepairClinicalData(dom, datasetsMap[dom]);
      window.clientAuditLogs[dom] = audit.auditLog || [];
    });
    switchTab('tab-qc');
    renderIssueManagementSystem();
  }
...
function renderSpecificationOnlyView(container, targetName, spec) {
      ...
      <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
        <div>
          <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
            <strong style="color:#fff; font-size:14.5px;">Specification: ${escapeHtml(targetName)}</strong>
            <span style="font-size:11px; font-weight:700; background:rgba(168,85,247,0.15); color:#c084fc; border:1px solid rgba(168,85,247,0.4); padding:3px 10px; border-radius:12px;">📐 ${vars.length} Variables Defined</span>
            <span style="font-size:11.5px; color:var(--text-muted);">Source: <strong>${escapeHtml(spec.fileName || 'Uploaded Specification')}</strong></span>
          </div>
          <div style="font-size:11.5px; color:var(--text-muted); margin-top:4px; line-height:1.5;">
            Study specification rules active. No patient data records loaded yet for <strong>${escapeHtml(targetName)}</strong>. Drop clinical data files (.sas7bdat, .xpt, .xlsx, .csv) above to execute automated validation against this specification.
          </div>
        </div>
        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
          <button class="btn-card-action" id="btn-download-spec-xlsx" style="background:linear-gradient(135deg, #7e22ce, #9333ea); color:#fff; font-weight:700; display:flex; align-items:center; gap:6px; cursor:pointer; border:none; box-shadow:0 2px 6px rgba(147,51,234,0.4);" title="Download domain specification workbook (.xlsx)">
            <span>📐</span> Download Spec (.xlsx)
          </button>
          <button class="btn-card-action secondary" id="btn-remove-spec" style="display:flex; align-items:center; gap:4px; font-size:11px; background:rgba(239,68,68,0.12); color:#f87171; border:1px solid rgba(239,68,68,0.3); cursor:pointer;" title="Remove this specification from agent">
            <span>🗑️</span> Remove Spec
          </button>
        </div>
      </div>
      <div style="display:flex; gap:12px; border-bottom:1px solid rgba(255,255,255,0.08); margin-top:14px; padding-bottom:0;">
        <button style="background:transparent; border:none; color:#c084fc; border-bottom:2.5px solid #c084fc; padding:8px 16px; font-weight:700; font-size:12.5px; cursor:pointer; display:flex; align-items:center; gap:6px;">
          <span>📐 Specification Variables &amp; CDISC Rules</span>
          <span style="font-size:11px; padding:2px 7px; border-radius:10px; background:rgba(168,85,247,0.25); color:#c084fc;">${vars.length} vars</span>
        </button>
      </div>
    </div>
  `;
  html += renderSpecificationTableHtml(targetName, vars, spec.fileName);
  container.innerHTML = html;

  const btnDl = document.getElementById('btn-download-spec-xlsx');
  if (btnDl) btnDl.addEventListener('click', () => downloadSpecificationAsExcel(targetName));
  const btnRm = document.getElementById('btn-remove-spec');
  if (btnRm) btnRm.addEventListener('click', (e) => removeLoadedSpecification(targetName, e));
}

function renderSpecificationTableHtml(domain, vars, sourceName) {
  if (!vars || vars.length === 0) {
    return `<div style="padding:28px; text-align:center; color:var(--text-muted); background:rgba(255,255,255,0.02); border-radius:8px;">No variable specifications defined for ${escapeHtml(domain)}.</div>`;
  }
  let html = `
    <div style="margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
      <div style="font-size:12px; color:var(--text-secondary);">
        Showing <strong>${vars.length}</strong> variable definition(s) from <em>${escapeHtml(sourceName || 'Specification')}</em>:
      </div>
    </div>
      ...
      <table class="data-table">
        <thead>
          <tr style="background:rgba(168,85,247,0.08);">
            ...
            <th style="min-width:120px;">Variable</th>
            <th style="min-width:220px;">Variable Label</th>
            <th style="min-width:85px;">Type</th>
            <th style="min-width:75px;">Length</th>
            <th style="min-width:95px; text-align:center;">Core / Req</th>
            <th style="min-width:160px;">Codelist / Format</th>
            <th style="min-width:240px;">Derivation / Origin / Algorithm</th>
          </tr>
        </thead>
        <tbody>
  `;
  ...
  vars.forEach((v, idx) => {
    ...
    html += `
      <tr>
        ...
        <td><code style="background:rgba(56,189,248,0.15); color:#38bdf8; padding:2px 6px; border-radius:4px; font-weight:700;">${escapeHtml(v.variable || '')}</code></td>
        <td style="font-size:12px; color:#fff; font-weight:500;">${escapeHtml(v.label || '')}</td>
        <td style="font-size:11.5px; color:var(--text-secondary); font-family:monospace;">${escapeHtml(v.type || 'Char')}</td>
        <td style="font-size:11.5px; color:var(--text-muted);">${escapeHtml(String(v.length || ''))}</td>
        <td style="text-align:center;"><span class="${coreBadgeClass}">${escapeHtml(v.core || 'Perm')}</span></td>
        <td style="font-size:11px; color:#e9d5ff;">${escapeHtml(v.codelist || '-')}</td>
        <td style="font-size:11px; color:var(--text-secondary); line-height:1.4;">${escapeHtml(v.derivation || v.origin || '-')}</td>
      </tr>
    `;
  });
...
function renderDatasetTable(dsetName) {
  ...
  const hasSpec = spec && Array.isArray(spec.variables) && spec.variables.length > 0;
  const specVarCount = hasSpec ? spec.variables.length : 0;

  if (!rows || rows.length === 0) {
    if (hasSpec) {
      window.currentDatasetSubView = 'SPEC';
      renderSpecificationOnlyView(container, targetName, spec);
      return;
    }
    ...
    container.innerHTML = `<div style="padding:36px 20px; text-align:center; color:var(--text-muted); background:rgba(255,255,255,0.02); border-radius:8px; border:1px dashed var(--border-subtle);">
      ...
      <strong style="color:#fff; font-size:14px;">No records or specifications currently loaded for ${escapeHtml(targetName)}.</strong>
      <p style="font-size:12px; margin-top:6px; max-width:540px; margin-left:auto; margin-right:auto; line-height:1.6;">
        Upload clinical data files (.sas7bdat, .xpt, .xlsx, .csv) into the <strong>Clinical Data</strong> zone above, or upload ADaM/SDTM specifications (.xlsx, .xml, .csv) into the <strong>Specifications</strong> zone.
      </p>
    </div>`;
    return;
  }
  ...
  let html = `
    ...
    <div style="background:rgba(255,255,255,0.03); border:1px solid var(--border-subtle); border-radius:8px; padding:14px 18px; margin-bottom:14px;">
      <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
        <div>
          <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
            ...
            ${hasSpec ? `<span style="font-size:11px; font-weight:700; background:rgba(168,85,247,0.15); color:#c084fc; border:1px solid rgba(168,85,247,0.4); padding:3px 10px; border-radius:12px;">📐 Spec Loaded (${specVarCount} vars)</span>` : ''}
          </div>
          <div style="font-size:11.5px; color:var(--text-muted); margin-top:3px; line-height:1.5;">
            Autonomous verification engine scanned every row, column, word, and character. Output is strictly separated into <strong>Clean Corrected Data</strong>, <strong>Discrepancies &amp; Auto-Repair Audit Report</strong>, and <strong>Specifications</strong>.
          </div>
        </div>
      ...
      <!-- Section Tabs: Clean Corrected Data vs. Errors Found & Fixed Audit Section vs. Specifications -->
      <div style="display:flex; gap:12px; border-bottom:1px solid rgba(255,255,255,0.08); margin-top:14px; padding-bottom:0;">
        ...
        <button id="tab-subview-spec" style="background:transparent; border:none; color:${currentSubView === 'SPEC' ? '#c084fc' : 'var(--text-muted)'}; border-bottom:${currentSubView === 'SPEC' ? '2.5px solid #c084fc' : '2.5px solid transparent'}; padding:8px 16px; font-weight:700; font-size:12.5px; cursor:pointer; display:flex; align-items:center; gap:6px;">
  ...
  if (currentSubView === 'DIFF' || window.clientDatasetViewMode === 'DIFF') {
    ...
    if (hasSpec) {
      ...
      const catVars = (entry && entry.keyVariables) ? entry.keyVariables.map((k, idx) => ({
        ...
        codelist: '-',
        derivation: `Standard CDISC ${targetName} model variable`
        ...
        core: idx < 4 ? 'Req' : 'Perm',
        codelist: '-',
        derivation: 'Derived from uploaded data header'
      }));
  ...
  const btnSpecXlsx = document.getElementById('btn-download-spec-xlsx');
  if (btnSpecXlsx) {
    btnSpecXlsx.addEventListener('click', () => {
      downloadSpecificationAsExcel(targetName);
    });
  }
...
function setupCommander() {
  const clinicalInput = document.getElementById('clinical-command-input');
  ...
  const handleCommand = () => {
    ...
    if (commanderInput && commanderInput.value.trim()) {
      const cmd = commanderInput.value.trim();
  ...
  if (clinicalInput) {
    clinicalInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        handleCommand();
      }
...
function setupTaskRadios() {
  ...
  // Setup In-Page Spec Drop Zone & PC File Browse for ADaM / SDTM Specifications
  const specDrop = document.getElementById('spec-drop-zone');
  const specInput = document.getElementById('spec-file-input');

  if (specDrop && specInput) {
    specDrop.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      specInput.click();
    });

    specInput.addEventListener('click', (e) => {
      e.stopPropagation();
    });

    specDrop.addEventListener('dragover', (e) => {
      e.preventDefault();
      e.stopPropagation();
      specDrop.classList.add('drag-over');
    });

    specDrop.addEventListener('dragenter', (e) => {
      e.preventDefault();
      e.stopPropagation();
      specDrop.classList.add('drag-over');
    });

    specDrop.addEventListener('dragleave', (e) => {
      e.preventDefault();
      e.stopPropagation();
      specDrop.classList.remove('drag-over');
    });
    ...
    specDrop.addEventListener('drop', (e) => {
      ...
      specDrop.classList.remove('drag-over');
      if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        handleSpecificationFiles(e.dataTransfer.files);
      }
    });

    specInput.addEventListener('change', () => {
      if (specInput.files && specInput.files.length > 0) {
        handleSpecificationFiles(specInput.files);
        specInput.value = '';
      }
    });
  }
  ...
  window.addEventListener('drop', (e) => {
    if (miniDrop && e.target !== miniDrop && !miniDrop.contains(e.target) &&
        specDrop && e.target !== specDrop && !specDrop.contains(e.target)) {
      e.preventDefault();
    }
  });
}
...
async function handleMiniFiles(files) {
  ...
  appendTerminalLog('STATE', 'UPLOAD', `Processing ${files.length} uploaded clinical data file(s) at ${getFormattedLocalTime()}...`);
...
// Ingestion Handler for ADaM & SDTM Specifications
async function handleSpecificationFiles(files) {
  if (!files || files.length === 0) return;
  const specStatus = document.getElementById('spec-upload-status');
  if (specStatus) {
    specStatus.innerHTML = `📐 Ingesting <strong>${files.length}</strong> spec file(s)... Extracting variable metadata & rules...`;
  }
  appendTerminalLog('STATE', 'SPEC_UPLOAD', `Processing ${files.length} uploaded ADaM/SDTM specification file(s) at ${getFormattedLocalTime()}...`);

  let loadedDomains = [];
  for (let i = 0; i < files.length; i++) {
    const doms = await processUploadedSpecificationFile(files[i]);
    if (doms && doms.length > 0) {
      loadedDomains.push(...doms);
    }
  }

  loadedDomains = [...new Set(loadedDomains)];

  if (specStatus) {
    if (loadedDomains.length > 0) {
      specStatus.innerHTML = `✅ <strong>${loadedDomains.length} specification(s) loaded!</strong> [${loadedDomains.join(', ')}]`;
    } else {
      specStatus.innerHTML = `✅ Specification processed.`;
    }
    setTimeout(() => { specStatus.innerHTML = ''; }, 7000);
  }
...
// Process Uploaded Specification File (Excel, Define-XML, CSV, JSON)
async function processUploadedSpecificationFile(file) {
  const fileName = file.name || 'specification.xlsx';
  const lower = fileName.toLowerCase();
  appendTerminalLog('STATE', 'SPEC_INGEST', `Ingesting specification ${fileName} (${(file.size/1024).toFixed(1)} KB)...`);
  ...
  try {
    if (lower.endsWith('.xlsx') || lower.endsWith('.xls')) {
      const buffer = await readFileAsArrayBuffer(file);
      if (typeof XLSX === 'undefined') throw new Error('SheetJS library initializing. Please retry.');
      ...
      wb.SheetNames.forEach(sheetName => {
        ...
        const dsetKey = Object.keys(rawRows[0]).find(k => /^(DATASET(_)?(NAME)?|DOMAIN|DSET|TABLE)$/i.test(k.trim()));

        if (dsetKey) {
          const groups = {};
          rawRows.forEach(row => {
            const d = String(row[dsetKey] || '').trim().toUpperCase();
            if (!d) return;
          ...
          Object.keys(groups).forEach(d => {
            const vars = normalizeSpecRowsToVariables(groups[d]);
            if (vars.length > 0) {
              window.clientSpecifications[d] = {
                domain: d,
                fileName: fileName,
                sheetName: sheetName,
                standard: d.startsWith('AD') ? 'ADaM' : 'SDTM',
                uploadTime: getFormattedLocalTime(),
                variables: vars
              };
              ensureDatasetPillExists(d);
              loadedDoms.push(d);
            }
          });
        } else {
          let inferredDomain = sheetName.trim().toUpperCase();
          if (/SHEET|VARIABLES|VARSPEC|SPEC/i.test(inferredDomain)) {
            const match = fileName.toUpperCase().match(/(AD[A-Z0-9]+|[A-Z]{2})/);
            if (match) inferredDomain = match[1];
            else inferredDomain = currentDatasetTab || 'ADSL';
          }
          const vars = normalizeSpecRowsToVariables(rawRows);
          if (vars.length > 0) {
            window.clientSpecifications[inferredDomain] = {
              domain: inferredDomain,
              fileName: fileName,
              sheetName: sheetName,
              standard: inferredDomain.startsWith('AD') ? 'ADaM' : 'SDTM',
              uploadTime: getFormattedLocalTime(),
              variables: vars
            };
            ensureDatasetPillExists(inferredDomain);
            loadedDoms.push(inferredDomain);
          }
        }
      });
      ...
      const itemGroupDefs = Array.from(doc.getElementsByTagName('ItemGroupDef'));
      const itemDefs = Array.from(doc.getElementsByTagName('ItemDef'));
      const itemDefMap = {};
      ...
      if (itemGroupDefs.length > 0) {
        itemGroupDefs.forEach(ig => {
          const domName = (ig.getAttribute('Domain') || ig.getAttribute('Name') || ig.getAttribute('OID') || '').toUpperCase();
          if (!domName) return;

          const itemRefs = Array.from(ig.getElementsByTagName('ItemRef'));
          const vars = [];
          itemRefs.forEach((ir, idx) => {
            const itemOid = ir.getAttribute('ItemOID') || ir.getAttribute('OID');
            const itemDef = itemDefMap[itemOid];
            const varName = itemDef ? (itemDef.getAttribute('Name') || itemOid) : itemOid;
            const descEl = itemDef ? itemDef.getElementsByTagName('TranslatedText')[0] : null;
            const label = descEl ? descEl.textContent.trim() : (itemDef?.getAttribute('Comment') || varName);
            const type = itemDef ? (itemDef.getAttribute('DataType') || 'Char') : 'Char';
            const len = itemDef ? itemDef.getAttribute('Length') : '';
            const isMandatory = ir.getAttribute('Mandatory') === 'Yes';
            const core = isMandatory ? 'Req' : (idx < 5 ? 'Exp' : 'Perm');
            ...
            vars.push({
              ...
              length: len,
              core: core,
              codelist: itemDef?.getAttribute('Origin') || '',
              derivation: itemDef?.getAttribute('Comment') || ''
            });
          });

          if (vars.length > 0) {
            window.clientSpecifications[domName] = {
              domain: domName,
              fileName: fileName,
              standard: domName.startsWith('AD') ? 'ADaM' : 'SDTM',
              uploadTime: getFormattedLocalTime(),
              variables: vars
            };
            ensureDatasetPillExists(domName);
            loadedDoms.push(domName);
          }
        });
      }
    } else if (lower.endsWith('.json')) {
      ...
      if (Array.isArray(parsed)) {
        const dom = (fileName.toUpperCase().match(/(AD[A-Z0-9]+|[A-Z]{2})/) || ['','ADSL'])[1];
        const vars = normalizeSpecRowsToVariables(parsed);
        window.clientSpecifications[dom] = {
          domain: dom,
          fileName: fileName,
          standard: dom.startsWith('AD') ? 'ADaM' : 'SDTM',
          uploadTime: getFormattedLocalTime(),
          variables: vars
        };
        ensureDatasetPillExists(dom);
        loadedDoms.push(dom);
      } else if (typeof parsed === 'object') {
        Object.keys(parsed).forEach(dom => {
          if (Array.isArray(parsed[dom])) {
            const d = dom.toUpperCase();
            const vars = normalizeSpecRowsToVariables(parsed[dom]);
            window.clientSpecifications[d] = {
              domain: d,
              fileName: fileName,
              standard: d.startsWith('AD') ? 'ADaM' : 'SDTM',
      ...
      if (parsed && parsed.rows && parsed.rows.length > 0) {
        const dsetKey = Object.keys(parsed.rows[0]).find(k => /^(DATASET(_)?(NAME)?|DOMAIN|DSET|TABLE)$/i.test(k.trim()));
        if (dsetKey) {
          const groups = {};
          parsed.rows.forEach(r => {
            const d = String(r[dsetKey] || '').trim().toUpperCase();
            if (!d) return;
            if (!groups[d]) groups[d] = [];
            groups[d].push(r);
          });
          ...
          let dom = fileName.replace(/\.[^/.]+$/, '').toUpperCase();
          const match = dom.match(/(AD[A-Z0-9]+|[A-Z]{2})/);
          if (match) dom = match[1];
          const vars = normalizeSpecRowsToVariables(parsed.rows);
          window.clientSpecifications[dom] = {
            domain: dom,
            fileName: fileName,
            standard: dom.startsWith('AD') ? 'ADaM' : 'SDTM',
    ...
    appendTerminalLog('WARN', 'SPEC_ERR', `Failed to parse specification ${fileName}: ${err.message}`);
  }
...
function normalizeSpecRowsToVariables(rawRows) {
  if (!rawRows || rawRows.length === 0) return [];
  const findKey = (row, regex) => Object.keys(row).find(k => regex.test(k.trim()));

  const sample = rawRows[0];
  const varKey = findKey(sample, /^(VAR(IABLE)?(_)?(NAME)?|ITEM(_)?(NAME)?|NAME|COLUMN)$/i) || 'VARIABLE';
  const lblKey = findKey(sample, /^(LABEL|VAR(IABLE)?(_)?LABEL|ITEM(_)?LABEL|DESCRIPTION|TITLE)$/i) || 'LABEL';
  const typKey = findKey(sample, /^(TYPE|DATA(_)?TYPE|FORMAT|VAR(IABLE)?(_)?TYPE)$/i) || 'TYPE';
  const lenKey = findKey(sample, /^(LENGTH|LEN|SIZE|BYTES)$/i) || 'LENGTH';
  const corKey = findKey(sample, /^(CORE|REQ(UIREMENT)?|ROLE|USAGE|MANDATORY)$/i) || 'CORE';
  const ctlKey = findKey(sample, /^(CODE(LIST)?|CONTROLLED(_)?TERMS?|CT|VALUES?)$/i) || 'CODELIST';
  const drvKey = findKey(sample, /^(DERIV(ATION)?|ALGORITHM|ORIGIN|SOURCE|METHOD|COMMENT|RULE)$/i) || 'DERIVATION';

  return rawRows.map((r, i) => {
    const vName = String(r[varKey] || r.Variable || r.variable || r.name || '').trim().toUpperCase();
    if (!vName) return null;
    return {
      seq: i + 1,
      variable: vName,
      label: String(r[lblKey] || r.Label || r.label || r.description || vName).trim(),
      type: String(r[typKey] || r.Type || r.type || 'Char').trim(),
      length: String(r[lenKey] || r.Length || r.length || '').trim(),
      core: String(r[corKey] || r.Core || r.core || 'Perm').trim(),
      codelist: String(r[ctlKey] || r.Codelist || r.codelist || '').trim(),
      derivation: String(r[drvKey] || r.Derivation || r.derivation || r.origin || '').trim()
    };
  }).filter(Boolean);
}

function downloadSpecificationAsExcel(domain) {
  ...
  let varList = spec ? spec.variables : [];

  if (!varList || varList.length === 0) {
    const catalog = window.CDISC_STANDARDS_CATALOG || [];
    const entry = catalog.find(c => c.code.toUpperCase() === dom);
    if (entry && entry.keyVariables) {
      varList = entry.keyVariables.map((k, idx) => ({
        variable: k,
        label: `${k} standard variable for ${entry.name}`,
        type: 'Char',
        length: 200,
        core: idx < 4 ? 'Req' : (idx < 8 ? 'Exp' : 'Perm'),
        codelist: '',
        derivation: `Standard CDISC ${dom} model variable`
      }));
    }
  }

  if (!varList || varList.length === 0) {
    alert(`No specification found for domain ${dom}`);
    return;
  }

  const exportRows = varList.map((v, i) => ({
    'Sequence': i + 1,
    'Dataset': dom,
    'Variable': v.variable || '',
    'Label': v.label || '',
  ...
  if (typeof XLSX !== 'undefined') {
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(exportRows);
    XLSX.utils.book_append_sheet(wb, ws, `${dom}_Specification`);
    XLSX.writeFile(wb, `${dom}_specification.xlsx`);
    appendTerminalLog('OK', 'DOWNLOAD', `Downloaded specification workbook ${dom}_specification.xlsx (${varList.length} variables).`);
  }
}
window.downloadSpecificationAsExcel = downloadSpecificationAsExcel;

function removeLoadedSpecification(domain, event) {
  if (event) {
  ...
  const dom = (domain || '').toUpperCase();
  if (!dom) return;
  if (window.clientSpecifications && window.clientSpecifications[dom]) {
    delete window.clientSpecifications[dom];
  }
  appendTerminalLog('WARN', 'SPEC_REMOVED', `Removed specification for domain ${dom}.`);
  showToastNotification(`🗑️ Specification for ${dom} removed.`);
  updateIngestionFilePills();
  renderDatasetTable(dom);
}
...
function updateIngestionFilePills() {
  ...
  const activeSpecs = window.clientSpecifications ? Object.keys(window.clientSpecifications).filter(k => 
    window.clientSpecifications[k] && Array.isArray(window.clientSpecifications[k].variables) && window.clientSpecifications[k].variables.length > 0
  ) : [];

  if (activeDomains.length === 0 && activeSpecs.length === 0) {
    container.innerHTML = `<div class="bridge-file-pill empty" style="color:var(--text-muted); border-style:dashed; font-style:italic;">
      No files loaded yet — upload your clinical data files or ADaM/SDTM specifications below
    </div>`;
    return;
  }
  ...
  activeSpecs.forEach(s => {
    const spec = window.clientSpecifications[s];
    const varCount = (spec && spec.variables) ? spec.variables.length : 0;
    html += `
      <div class="bridge-file-pill spec-pill" title="${escapeHtml(s)} Specification (${varCount} Variables)" style="display:inline-flex; align-items:center; gap:6px; padding:4px 10px; border-radius:14px; background:rgba(168,85,247,0.14); border:1px solid rgba(168,85,247,0.45); margin:3px 4px 3px 0;">
        <span style="font-size:12px;">📐</span> 
        <strong style="color:#e9d5ff;">${escapeHtml(s)} Spec</strong> 
        <span style="color:#d8b4fe; font-size:11px;">(${varCount} vars)</span>
        <button class="btn-remove-pill" onclick="removeLoadedSpecification('${escapeHtml(s)}', event)" title="Remove ${escapeHtml(s)} specification" style="background:rgba(239,68,68,0.2); border:1px solid rgba(239,68,68,0.4); color:#fca5a5; border-radius:50%; width:18px; height:18px; font-size:12px; font-weight:700; line-height:1; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; margin-left:4px; transition:all 0.15s ease;">×</button>
      </div>
    `;
  });
...
// =========================================================
// UNIVERSAL CDISC DOMAIN ACCESSOR & CATEGORY FILTER ENGINE
// =========================================================
